...
"""
function xadd(con, key::AbstractString, id, field_values...; nomkstream=false, maxlen=nothing, minid=nothing, limit=nothing, appx=true)
    local args = [key]
    nomkstream &&
        push!(args, "nomkstream")
...
function Redis.convert_response(::Type{NamedTuple}, v::Vector{String})
    return (; (Symbol(v[i]) => v[i+1] for i in 1:2:length(v))...)
end
